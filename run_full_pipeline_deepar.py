"""
GPU 서버용 전체 파이프라인 (DeepAR 버전)
  1단계: Optuna 30 trials → 최적 하이퍼파라미터 탐색
  2단계: 최적 파라미터로 재학습
  3단계: Rolling Window 백테스트 + Kupiec 결과 출력

실행:
  python run_full_pipeline_deepar.py
  python run_full_pipeline_deepar.py --n_trials 50   # trial 수 조절
  python run_full_pipeline_deepar.py --skip_optuna   # Optuna 생략, config.yaml 값으로 바로 학습

NOTE: DeepAR은 distribution 파라미터를 출력하는 autoregressive 모델.
      본 파이프라인은 NormalDistributionLoss로 학습하지만, AdaptivePinballLoss를
      위해 매 step마다 to_quantiles()로 quantile을 analytic 계산한 뒤 사용한다.
      따라서 학습이 다른 모델보다 약간 느리지만 알고리즘 일관성은 유지된다.
"""
...
def run_optuna(config: dict, n_trials: int) -> dict:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trainer = pl.Trainer(
            ...
            logger=pl.loggers.CSVLogger(LOG_DIR, name="optuna_trials_deepar"),
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="m3_full_model_deepar",
    ...
    print("\n" + "=" * 50)
    print("[Optuna 완료 - DeepAR] 최적 파라미터")
...
def train_with_params(config: dict, params: dict) -> Path:
    ...
    model = M3FullModel.from_dataset(
        ...
        rnn_layers=params.get("rnn_layers", model_cfg.get("rnn_layers", 2)),
        cell_type=params.get("cell_type", model_cfg.get("cell_type", "LSTM")),
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="m3_deepar_best_{epoch:02d}_{val_loss:.4f}",
        monitor="val_loss",
        save_top_k=1,
        mode="min",
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.CSVLogger(LOG_DIR, name="m3_final_deepar"),
        gradient_clip_val=0.1,
        accelerator="gpu",
        devices=1,
    )
...
def run_backtest(config: dict) -> None:
    ...
    print("\n" + "=" * 60)
    print("[백테스트 결과 - DeepAR] Rolling Window 5-Fold Kupiec POF Test")
    print("=" * 60)
    ...
    results_path = Path("model/saved/backtest_results_deepar.csv")
...
def main():
    ...
    if args.skip_optuna:
        ...
        params = {
            ...
            "cell_type": model_cfg.get("cell_type", "LSTM"),
    ...
    print("\n" + "=" * 60)
    print("[COVID 구간 백테스트 결과 - DeepAR] Kupiec POF Test")
    print("=" * 60)
    ...
    covid_results_path = Path("model/saved/covid_backtest_results_deepar.csv")
