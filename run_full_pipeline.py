...
실행:
  ...
  python run_full_pipeline.py --n_trials 50   # trial 수 조절
  python run_full_pipeline.py --skip_optuna   # Optuna 생략, config.yaml 값으로 바로 학습
"""
...
from validation.backtest.backtest import (
...
LOG_DIR = Path("logs")
BEST_PARAMS_PATH = Path("model/saved/best_params.json")
...
def train_with_params(config: dict, params: dict) -> Path:
    ...
    model = M3FullModel.from_dataset(
        ...
        attention_head_size=params.get("attention_head_size", model_cfg["attention_head_size"]),
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="m3_best_{epoch:02d}_{val_loss:.4f}",
        monitor="val_loss",
        save_top_k=1,
        mode="min",
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.CSVLogger(LOG_DIR, name="m3_final"),
        gradient_clip_val=0.1,
        accelerator="gpu",
        devices=1,
    )
...
def run_backtest(config: dict) -> None:
    ...
    print("\n" + "=" * 60)
    print("[백테스트 결과] Rolling Window 5-Fold Kupiec POF Test")
    print("=" * 60)
    ...
    results_path = Path("model/saved/backtest_results.csv")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("[COVID 구간 백테스트 결과] Kupiec POF Test")
    print("=" * 60)
    ...
    covid_results_path = Path("model/saved/covid_backtest_results.csv")
    ...
    print("\n" + "=" * 60)
    print("[우크라이나 전쟁 + 인플레이션 구간 백테스트 결과] Kupiec POF Test")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("[트럼프 관세 충격 구간 백테스트 결과] Kupiec POF Test")
    print("=" * 60)
