"""
GPU 서버용 전체 파이프라인 (TimeXer 버전)
  1단계: Optuna 30 trials → 최적 하이퍼파라미터 탐색
  2단계: 최적 파라미터로 재학습
  3단계: Rolling Window 백테스트 + Kupiec 결과 출력

실행:
  python run_full_pipeline_timexer.py
  python run_full_pipeline_timexer.py --n_trials 50
  python run_full_pipeline_timexer.py --skip_optuna
"""
...
from validation.backtest.timexer_backtest import rolling_window_backtest, covid_backtest

CONFIG_PATH = Path("configs/config.yaml")
CHECKPOINT_DIR = Path("model/saved")
LOG_DIR = Path("logs")
BEST_PARAMS_PATH = Path("model/saved/best_params_timexer.json")
...
def run_optuna(config: dict, n_trials: int) -> dict:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        patch_length = trial.suggest_categorical("patch_length", [6, 10, 12, 15])
        ...
        trainer = pl.Trainer(
            ...
            logger=pl.loggers.CSVLogger(LOG_DIR, name="optuna_trials_timexer"),
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="m3_full_model_timexer",
    ...
    print("\n" + "=" * 50)
    print("[Optuna 완료 - TimeXer] 최적 파라미터")
...
def train_with_params(config: dict, params: dict) -> Path:
    ...
    model = M3FullModel.from_dataset(
        ...
        n_heads=params.get("n_heads", model_cfg.get("n_heads", 4)),
        e_layers=params.get("e_layers", model_cfg.get("e_layers", 2)),
        d_ff=params.get("d_ff", model_cfg.get("d_ff", 256)),
        patch_length=params.get("patch_length", model_cfg.get("patch_length", 12)),
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="m3_timexer_best_{epoch:02d}_{val_loss:.4f}",
        monitor="val_loss",
        save_top_k=1,
        mode="min",
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.CSVLogger(LOG_DIR, name="m3_final_timexer"),
        gradient_clip_val=0.1,
        accelerator="gpu",
        devices=1,
    )
...
def run_backtest(config: dict) -> None:
    ...
    print("\n" + "=" * 60)
    print("[백테스트 결과 - TimeXer] Rolling Window 5-Fold Kupiec POF Test")
    print("=" * 60)
    ...
    results_path = Path("model/saved/backtest_results_timexer.csv")
...
def main():
    ...
    if args.skip_optuna:
        ...
        params = {
            ...
            "patch_length": model_cfg.get("patch_length", 12),
    ...
    print("\n" + "=" * 60)
    print("[COVID 구간 백테스트 결과 - TimeXer] Kupiec POF Test")
    print("=" * 60)
    ...
    covid_results_path = Path("model/saved/covid_backtest_results_timexer.csv")
    ...
    print("\n" + "=" * 60)
    print("[우크라이나 전쟁 + 인플레이션 구간 백테스트 결과 - TimeXer] Kupiec POF Test")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("[트럼프 관세 충격 구간 백테스트 결과 - TimeXer] Kupiec POF Test")
    print("=" * 60)
