"""
GPU 서버용 전체 파이프라인 (DecoderMLP 버전)
  1단계: Optuna 30 trials → 최적 하이퍼파라미터 탐색
  2단계: 최적 파라미터로 재학습
  3단계: Rolling Window 백테스트 + Kupiec 결과 출력

실행:
  python run_full_pipeline_decodermlp.py
  python run_full_pipeline_decodermlp.py --n_trials 50   # trial 수 조절
  python run_full_pipeline_decodermlp.py --skip_optuna   # Optuna 생략, config.yaml 값으로 바로 학습
"""
...
from model.m3_full_model.decodermlp_model import M3FullModel
from validation.backtest.decodermlp_backtest import rolling_window_backtest, covid_backtest

CONFIG_PATH = Path("configs/config.yaml")
CHECKPOINT_DIR = Path("model/saved")
LOG_DIR = Path("logs")
BEST_PARAMS_PATH = Path("model/saved/best_params_decodermlp.json")
...
def run_optuna(config: dict, n_trials: int) -> dict:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trainer = pl.Trainer(
            ...
            logger=pl.loggers.CSVLogger(LOG_DIR, name="optuna_trials_decodermlp"),
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="m3_full_model_decodermlp",
    ...
    print("\n" + "=" * 50)
    print("[Optuna 완료 - DecoderMLP] 최적 파라미터")
...
def train_with_params(config: dict, params: dict) -> Path:
    ...
    model = M3FullModel.from_dataset(
        ...
        n_hidden_layers=params.get("n_hidden_layers", model_cfg.get("n_hidden_layers", 3)),
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="m3_decodermlp_best_{epoch:02d}_{val_loss:.4f}",
        monitor="val_loss",
        save_top_k=1,
        mode="min",
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.CSVLogger(LOG_DIR, name="m3_final_decodermlp"),
        gradient_clip_val=0.1,
        accelerator="gpu",
        devices=1,
    )
...
def run_backtest(config: dict) -> None:
    ...
    print("\n" + "=" * 60)
    print("[백테스트 결과 - DecoderMLP] Rolling Window 5-Fold Kupiec POF Test")
    print("=" * 60)
    ...
    results_path = Path("model/saved/backtest_results_decodermlp.csv")
...
def main():
    ...
    if args.skip_optuna:
        ...
        params = {
            ...
            "n_hidden_layers": model_cfg.get("n_hidden_layers", 3),
    ...
    print("\n" + "=" * 60)
    print("[COVID 구간 백테스트 결과 - DecoderMLP] Kupiec POF Test")
    print("=" * 60)
    ...
    covid_results_path = Path("model/saved/covid_backtest_results_decodermlp.csv")
    ...
    print("\n" + "=" * 60)
    print("[우크라이나 전쟁 + 인플레이션 구간 백테스트 결과 - DecoderMLP] Kupiec POF Test")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("[트럼프 관세 충격 구간 백테스트 결과 - DecoderMLP] Kupiec POF Test")
    print("=" * 60)
