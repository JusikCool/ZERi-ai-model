...
class M3FullModel(pl.LightningModule):
    ...
    def from_dataset(
        ...
        if n_blocks is not None:
            nhits_kwargs["n_blocks"] = n_blocks
        if pooling_sizes is not None:
            nhits_kwargs["pooling_sizes"] = pooling_sizes
        if downsample_frequencies is not None:
            nhits_kwargs["downsample_frequencies"] = downsample_frequencies

        nhits = NHiTS.from_dataset(dataset, **nhits_kwargs)
