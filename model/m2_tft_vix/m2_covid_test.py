...
covid_time_idx = df[
    (df["Date"] >= "2020-01-01") & (df["Date"] < "2021-01-01")
]["time_idx"].unique()

if len(covid_time_idx) == 0:
    raise ValueError(f"COVID 구간({"2020-01-01"}~{"2021-01-01"}) 데이터 없음")

min_time = int(covid_time_idx.min())
max_time = int(covid_time_idx.max())
fold_size = (max_time - min_time) // (5 + 1)

fold_df = df[
    (df["time_idx"] >= min_time - 60)
    & (df["time_idx"] <= max_time)
].copy()
...
report["period"] = f"{"2020-01-01"} ~ {"2021-01-01"}"
