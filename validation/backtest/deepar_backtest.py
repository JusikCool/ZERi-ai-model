...
BEST_PARAMS_PATH = CHECKPOINT_DIR / "best_params_deepar.json"
COVID_START = "2020-01-01"
COVID_END = "2021-01-01"


def _load_best_params() -> dict:
    """학습 시 저장된 DeepAR best params 로드. 없으면 빈 dict."""
...
def _build_model_kwargs(model_cfg: dict, vix_mean: float, vix_std: float) -> dict:
    """
    DeepAR용 from_dataset kwargs 빌더.
    우선순위: best_params_deepar.json > config["model"]
    """
    ...
    for opt_key in ("rnn_layers", "cell_type"):
        if opt_key in best:
...
def _find_deepar_checkpoint() -> Path:
    """가장 최근의 DeepAR 체크포인트 찾기."""
    ckpts = glob.glob(str(CHECKPOINT_DIR / "m3_deepar_best_*.ckpt"))
    if not ckpts:
        ckpts = glob.glob(str(CHECKPOINT_DIR / "*.ckpt"))
    if not ckpts:
        raise FileNotFoundError(f"{CHECKPOINT_DIR} 에 DeepAR 체크포인트가 없습니다.")
...
def rolling_window_backtest(
    ...
    print(f"[DeepAR 백테스트] 체크포인트 로드: {ckpt_path}")
    ...
    print(f"[DeepAR 백테스트] hidden_size={model_kwargs.get('hidden_size')}, "
          f"rnn_layers={model_kwargs.get('rnn_layers')}, "
          f"cell_type={model_kwargs.get('cell_type')}")
...
def covid_backtest(
    ...
    ckpt_path = _find_deepar_checkpoint()
    print(f"[DeepAR COVID 백테스트] 체크포인트 로드: {ckpt_path}")
