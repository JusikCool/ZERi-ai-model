...
BEST_PARAMS_PATH = CHECKPOINT_DIR / "best_params_decodermlp.json"
...
def _load_best_params() -> dict:
    """학습 시 저장된 DecoderMLP best params 로드. 없으면 빈 dict."""
...
def _build_model_kwargs(model_cfg: dict, vix_mean: float, vix_std: float) -> dict:
    """
    DecoderMLP용 from_dataset kwargs 빌더.
    우선순위: best_params_decodermlp.json > config["model"]
    ...
    for opt_key in ("n_hidden_layers", "activation_class", "norm"):
        if opt_key in best:
...
def _find_decodermlp_checkpoint() -> Path:
    """가장 최근의 DecoderMLP 체크포인트 찾기."""
    ckpts = glob.glob(str(CHECKPOINT_DIR / "m3_decodermlp_best_*.ckpt"))
    if not ckpts:
        ckpts = glob.glob(str(CHECKPOINT_DIR / "*.ckpt"))
    if not ckpts:
        raise FileNotFoundError(f"{CHECKPOINT_DIR} 에 DecoderMLP 체크포인트가 없습니다.")
...
def rolling_window_backtest(
    ...
    ckpt_path = _find_decodermlp_checkpoint()
    print(f"[DecoderMLP 백테스트] 체크포인트 로드: {ckpt_path}")
    ...
    print(f"[DecoderMLP 백테스트] hidden_size={model_kwargs.get('hidden_size')}, "
          f"n_hidden_layers={model_kwargs.get('n_hidden_layers')}")
...
def period_backtest(
    ...
    ckpt_path = _find_decodermlp_checkpoint()
    print(f"[DecoderMLP COVID 백테스트] 체크포인트 로드: {ckpt_path}")
