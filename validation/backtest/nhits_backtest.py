...
BEST_PARAMS_PATH = CHECKPOINT_DIR / "best_params_nhits.json"
...
def _load_best_params() -> dict:
    """학습 시 저장된 NHiTS best params 로드. 없으면 빈 dict."""
...
def _build_model_kwargs(model_cfg: dict, vix_mean: float, vix_std: float) -> dict:
    """
    NHiTS용 from_dataset kwargs 빌더.
    우선순위: best_params_nhits.json > config["model"]
    ...
    for opt_key in ("n_layers", "n_blocks", "pooling_sizes", "downsample_frequencies"):
        if opt_key in best:
...
def _find_nhits_checkpoint() -> Path:
    """가장 최근의 NHiTS 체크포인트 찾기."""
    ckpts = glob.glob(str(CHECKPOINT_DIR / "m3_nhits_best_*.ckpt"))
    if not ckpts:
        ckpts = glob.glob(str(CHECKPOINT_DIR / "*.ckpt"))
    if not ckpts:
        raise FileNotFoundError(f"{CHECKPOINT_DIR} 에 NHiTS 체크포인트가 없습니다.")
...
def rolling_window_backtest(
    ...
    ckpt_path = _find_nhits_checkpoint()
    print(f"[NHiTS 백테스트] 체크포인트 로드: {ckpt_path}")
    ...
    print(f"[NHiTS 백테스트] hidden_size={model_kwargs.get('hidden_size')}, "
          f"n_layers={model_kwargs.get('n_layers')}")
...
def period_backtest(
    ...
    ckpt_path = _find_nhits_checkpoint()
    print(f"[NHiTS COVID 백테스트] 체크포인트 로드: {ckpt_path}")
